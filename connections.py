...
def startSakis():
    ...
    connection, addr = checkIfconfig("wwan0")
    return connection
...
class Connect():
    ...
    def doConnect(self):
        ...
        if "eth0" in interfaces:
            connection, addr = checkIfconfig("eth0")
        if addr == "" and "wlan0" in interfaces:
            connection, addr = checkIfconfig("wlan0")
        ...
        if addr == "" and "eth1" in interfaces:
            try:
                ...
                logging.warning("%s Exception: %s %s", ModuleName, type(ex), str(ex.args))
            connection, addr = checkIfconfig("eth1")
        ...
        if addr == "" and "wwan0" in interfaces:
            ...
            d1 = threads.deferToThread(startSakis)
    ...
    def checkConnected(self, connection):
        ...
        logging.info("%s checkConnected. Connected by: %s", ModuleName, self.connection)
    
    def checkWifiConnected(self):
        connection, addr = checkIfconfig("wlan0")
        logging.info("%s checkWifiConnected. Address: %s", ModuleName, addr)
